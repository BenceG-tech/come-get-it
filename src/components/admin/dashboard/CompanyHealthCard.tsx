  ...
  useEffect(() => {
    (async () => {
      ...
      const [partners, leadsNew, leadsPrev, posts, postsPrev, docs, docsPrev, outreach, outreachPrev] = await Promise.all([
        ...
        supabase.from("marketing_calendar").select("id", { count: "exact", head: true }).eq("status", "published").gte("scheduled_at", weekAgo),
        supabase.from("marketing_calendar").select("id", { count: "exact", head: true }).eq("status", "published").gte("scheduled_at", twoWeeksAgo).lt("scheduled_at", weekAgo),
