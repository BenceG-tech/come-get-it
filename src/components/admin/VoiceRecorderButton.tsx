    ...
    try {
      ...
      if (intent === "task") {
        const { data } = await supabase.from("checklist_items").insert({
          title, description: body, status: "open", priority: 2, owner: user.id,
        }).select().single();
        ...
        const { data } = await supabase.from("saved_content_snippets").insert({
          title, content: body, content_type: "idea", user_id: user.id, tags: structured.tags ?? [],
        }).select().single();
        ...
        // generic interaction without partner_id — user can later assign
        const { data } = await supabase.from("partner_interactions").insert({
          channel: "voice_note", notes: `${title}\n\n${body}`, user_id: user.id, partner_id: null as any,
        }).select().maybeSingle();
        target_table = "partner_interactions"; target_id = data?.id ?? null;
      }
      await supabase.from("voice_notes").update({
        status: "applied", intent, target_table, target_id,
        structured: { ...structured, title, body, intent },
      }).eq("id", noteId);
